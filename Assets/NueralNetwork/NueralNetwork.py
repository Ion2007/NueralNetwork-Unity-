...
class graph:
  def __init__(self):
    ...
    self.epochs=0
  def makeNet(self):
    f = open("Assets/NueralNetwork/input.txt", "r")
    hi= f.readlines()
    self.learning_rate=-1*float(hi[len(hi)-1].split()[1])
    self.epochs=int(hi[len(hi)-1].split()[0])
    for i in range(0,len(hi)-1):
      li=[]
      for j in range(int(hi[i].split()[0].strip('​'))):
        if i==0:
...
print(scene.network)
f = open("Assets/NueralNetwork/out.txt", 'w')
for j in range(scene.epochs):
  ...
  scene.backword()
  f.write("Epoch: "+str(j)+"/"+str(scene.epochs)+", "+"loss: "+str(scene.loss)+'\n')
  print(scene.out)
  print(scene.loss)
f.close()  
print(scene.out)
